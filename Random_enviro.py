...
def spawn_shroom(x,y,chunknode):
    Entity(model="plumpshrooms", texture='shroom_color2', position=(x * 10, -.6, y * 10), parent=chunknode)
    Entity(model="plumpshrooms", texture='shroom_color2', position=((x * 10)-3, -.6, y * 10), parent=chunknode)
    Entity(model="plumpshrooms", texture='shroom_color2', position=((x * 10)-3, -.6, (y * 10)-3), parent=chunknode)

def spawn_oak(x,y,chunknode):
    Entity(model="lowpolytree", texture='normtreetex', position=(x * 5, 0, y * 5), parent=chunknode)
def spawn_pine(x,y,chunknode):
    Entity(model="lowpolypine", texture='backed', position=(x*5, 0, y*5), parent=chunknode)
